...
Planned layout. Exists today (after Step 3): root config, `packages/{config,domain,contracts,modules,db,api,app-core,i18n,tokens}`, `apps/web` (root layout, `(auth)` pages login/signup/verify/forgot/reset, `(app)` pages home and account, `not-found`/`error`, `proxy.ts`, `app/api/trpc/[trpc]/route.ts`, `src/{components,features,lib}`), `supabase/` (incl. `templates/`), `tests/e2e`, `docs/`, `.github/workflows/ci.yml`. The rest is created by the step that needs it.
...
- **No account-existence leak in the screens** (D-062). A registered and a new address see the same screens and, for sign-up and "send me a code", get the same Auth answer: `[auth.sms] enable_confirmations = true` makes Supabase answer an existing email's sign-up like a new one, and code requests create missing accounts. `signUp` with an existing email sends no code, so the sign-up code screen links to sign-in and reset. Still revealed by Supabase's public endpoints (accepted, listed in D-062): the `/recover` per-address rate limit and response timing, and `email_exists` on an authenticated email change. CAPTCHA (Turnstile) on sign-up, code and reset before the first real user. Supabase errors map to i18n keys by `error.code`, never by message text (unknown → `errors.internal`).
...
- **Web session** (D-069): `@supabase/ssr` cookies, `Secure` except on plain http to a host other than localhost (`secureSessionCookies`), `SameSite=Lax`. `proxy.ts` refreshes the session (keeping the refresh's cookies and no-store headers on redirects), sends signed-out visitors to `/login` and signed-in users away from the auth pages (`/reset` is open to both). A code request hands only the email, purpose and send time to `/verify` or `/reset` through sessionStorage (for `/reset` also whether the code was already used and whether a new password is required, so a reload never offers a password change).
...
- The service/secret key is server-only, used for exactly: Auth admin (today: account deletion, `packages/api/src/admin`), Storage signed URLs, background jobs. Sentry scrubs cost values. The AI layer [Later] only creates drafts a human confirms, and its output passes through redaction.
...
- As built (Step 3): the web (app) layout loads `me` through the server caller and seeds it into a `QueryClient` created per mount (`ApiProvider`, `httpBatchLink` with `maxItems` = `API_MAX_BATCH_SIZE`); there is no business context yet, so the per-business key starts with Step 5. Sign-out reloads the page, which drops the cache.
...
- **Next server:** one i18next instance per request via `createInstance()` inside React `cache()`. Never a global `changeLanguage` on the server (lint rule). The root layout sets `<html lang dir>` from the request's locale, so there is no flash, and passes the locale to the client `I18nProvider`, which creates its own instance. Today the browser bundle holds every namespace of both languages (~24 KB gzip); passing only the needed namespaces of one language is deferred until Steps 5–6 add namespaces (D-069).
