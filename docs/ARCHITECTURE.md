...
Planned layout. Exists today (after Step 5): root config, `packages/{config,domain,contracts,modules,db,api,app-core,i18n,tokens}`, `apps/web` (root layout, `(auth)` pages login/signup/verify/forgot/reset, `(app)` pages home, account, setup and `b/[businessId]`, `not-found`/`error`, `proxy.ts`, `app/api/trpc/[trpc]/route.ts`, `src/{components,features,lib}`), `supabase/` (incl. `templates/`), `tests/e2e`, `docs/`, `.github/workflows/ci.yml`. The rest is created by the step that needs it.
