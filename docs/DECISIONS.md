...
- **Status:** every entry is DECIDED. Implemented so far: the Step 0a repo scaffold, the Step 1 data foundation (D-047–D-053 and the data/tenancy entries they build on), the Step 2 API core (D-054–D-061), the Step 3 web auth (D-062–D-071), the owner's password rule (D-072), the "too soon" fix (D-073) and Step 5 businesses & Smart Setup (D-074–D-079). "Open:" marks a sub-choice that is still unsettled, tracked in ROADMAP.md §Open, with deadline.
