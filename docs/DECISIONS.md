...
- **Status:** every entry is DECIDED. Implemented so far: the Step 0a repo scaffold, the Step 1 data foundation (D-047–D-053 and the data/tenancy entries they build on), the Step 2 API core (D-054–D-061), the Step 3 web auth (D-062–D-071), the owner's password rule (D-072), the "too soon" fix (D-073), Step 5 businesses & Smart Setup (D-074–D-079), Step 6 web settings (D-080–D-088) and Step 7 the web shell (D-089–D-096). "Open:" marks a sub-choice that is still unsettled, tracked in ROADMAP.md §Open, with deadline.
...
## Businesses & Smart Setup (Step 5)

### D-074 · 2026-09-25 · Smart Setup is versioned data and pure functions in `@bizcost/modules`

- **Decision:** The question set (`QUESTION_SET_VERSION = 1`: 10 questions, each with a type, options, `showIf` skip logic over earlier answers and i18n keys), `normalizeAnswers`/`parseSetupAnswers`, `recommend()`, `applyAdjustments`/`toggleSetupItem` and the review data (`buildSetupReview`, `setupTexts`) live in `packages/modules/src/setup`, not in `domain`: they read the module manifests and the capability registry, which `domain` cannot import. `modules` imports `@bizcost/i18n` for key types only. The web wizard and the API call the same functions; the server parses the answers strictly and recomputes everything. The business name is step 0 and not counted; skip logic shows 5–10 questions (5 for a solo home project business, 10 for a maker in a shop with a team). The wording is simple MSA (D-067) and waits for the owner's confirmation (PRODUCT.md §6.12).
- **Why:** The lead's G1: one source for the client's live review and the server's decision, and a place where the manifests are in reach. Pure data makes the fast-check invariants (§6.10) possible over every valid answer walk.
- **Rejected:** `recommend()` in `domain` (it would need a copy of the module data); questions served by the API (a round trip per screen, no offline-first wizard); trusting a recommendation the client sends. Details: PRODUCT.md §6; ARCHITECTURE.md §Repo structure

### D-075 · 2026-09-25 · The review lists planned modules honestly

- **Decision:** The review ("Here's your BizCost") lists every module the setup turns on, also the planned ones, because the setup is saved now and each appears by itself when it is released. While every listed item is planned (all of M1), one banner says so and no row has a tag; once released and planned items are mixed, planned rows get a small "Soon" / «قريبًا» tag and the banner goes. These are rows of the review, never pages, nav items or links, and the business home shows only the capability statements. The "cost of each job" row (`jobs_and_tasks`) counts as planned (Phase 5).
- **Why:** The lead's G3. Hiding the planned modules would make the review empty in M1 and the choices invisible; showing them as if they worked would mislead.
- **Rejected:** hiding planned modules from the review; placeholder pages behind them (D-003). Details: PRODUCT.md §6.7

### D-076 · 2026-09-25 · Creating a business: one transaction, idempotent, at most 10 a day

- **Decision:** `business.createFromSetup` (authed, not business-scoped) writes everything in ONE `withTenantTx` of the new business (ARCHITECTURE.md §API & request flow). It is idempotent on the client's `businessId`: `setup_answers.request_hash` holds a SHA-256 fingerprint of the canonical payload (answers as normalized sets, adjustments in registry order, name, locale, version). The same payload again returns the same business; so does a race of two identical requests, whose loser gets a unique violation or, for the last business the daily limit allows, the limit error, and is answered from the saved fingerprint. Another payload for an existing business, or an id of a business the caller cannot see, is CONFLICT. `app.create_business()` allows a user at most 10 businesses in 24 hours (soft-deleted ones count; a transaction-level advisory lock per user makes concurrent creations count each other; SQLSTATE `BZ429` → `rate_limited`). Names (business and display name) refuse control characters in the DTOs, and SQLSTATE 22021/22P05 map to VALIDATION as a backstop; the web forms turn a pasted tab or line break into a space.
- **Why:** The lead's G4. A lost reply must not create a second business, and a changed setup must not be taken for the first one. A limit in the only creation function cannot be skipped by any API path; creating and deleting cannot get around it. A NUL in a name was a 500 and an error report.
- **Rejected:** checking the limit only in the API (another path could skip it); an idempotency table (the setup row already exists once per business); counting only live businesses. Details: ARCHITECTURE.md §API & request flow; DATA_MODEL.md §2, §4 setup_answers

### D-077 · 2026-09-25 · Where the app goes after sign-in: `profiles.last_business_id`

- **Decision:** `/` sends a user with a business to `/b/[last_business_id]` while they are still an active member of that live business, else to their first business; without one, home offers Smart Setup. The id is set by `createFromSetup` and by `account.setLastBusiness` (authed; FORBIDDEN for a business the caller is not an active member of), which the web calls once when a business page opens with another id. A business link has one spelling: the business layout redirects an id in capitals to lowercase, the form `me`, the switcher and the API use.
- **Why:** The lead's G4 asked to pick one: an explicit mutation keeps `business.context` a pure read (it is loaded by every business page and prefetch). A link typed in capitals showed an empty page because the page compared ids as typed.
- **Rejected:** writing it inside `business.context` (a read with a write, repeated on every refresh); a cookie (shared across tabs, ARCHITECTURE.md §Active business). Details: ARCHITECTURE.md §Active business

### D-078 · 2026-09-25 · The review follows the switches; spec refinements from the Step 5 review

- **Decision:** (1) Reasons and off notes are computed for the current switches (`setupTexts`: the §6.6 rules read the answers with the capabilities, VAT and modules as switched), so no row contradicts a statement; a recommended module whose variants no longer fit shows its description. (2) A capability switched back on also turns back on the recommended modules that switching it off turned off, so off-and-on again gives the recommendation back. (3) "More sections you can add" hides modules that clearly do not fit: Projects is not offered to food businesses, to businesses that only resell, or to someone working alone from home. (4) The business home states team and VAT always, and branches only when on (a home business is never asked about branches). (5) Next goes first to an unanswered question before the current one, otherwise to the next shown question in order, so unanswered questions later on and options a change made visible are still seen; Confirm goes to the first unanswered question. (6) A Confirm refused with CONFLICT for a business of this user (an earlier Confirm created it before its reply was lost, or a duplicated tab did) never shows "ready": the review says the business was already created with the earlier answers and offers "Open this business" or "Create a new business with these answers" (a new id). (7) Copy: the POS hint no longer assumes a POS, the "already covered" hint says what is covered, the workshop jobs reason says "job" throughout, and the Arabic home button and `catalog` option read naturally.
- **Why:** Findings of the Step 5 security, spec and UX review: a VAT-off workshop was still told it is "registered for VAT"; switching Team off and on silently dropped Attendance, Payroll and Petty Cash; a home baker was offered Projects and told "One branch"; a changed name after a lost reply showed "ready" for the old one. (5) keeps the builder's walk instead of PRODUCT.md's earlier "Next jumps to the first unanswered question", which skipped screens whose new options matter (e.g. "in my shop" after moving to a shop); the lead or the owner can reverse it in `stepAfter`.
- **Rejected:** texts from the answers only; restoring from a history of the user's switches (the recommendation is the documented default); per-row "not for you" lists for every module (only Projects was reported). Details: PRODUCT.md §6.3, §6.7, §6.8

### D-079 · 2026-09-25 · Shape of the business pages on the web

- **Decision:** `b/[businessId]/layout.tsx` loads `me` and `business.context` through the server caller (`getBusinessContext`; not a member or no such business → not found) and mounts `BusinessApiProvider` keyed by the business id: its own `QueryClient` and a tRPC client that sends `x-business-id` on every request, seeded with both. The business home is a welcome card with the name and the capability statements (`businessSummaryKeys`, with the terminology profile from `business.context`); in a business the header has the business switcher (memberships from `me`; "Create another business" → `/setup`) and, on phones, the logo mark and an icon-only language button (44 px targets). The wizard keeps its draft (name, raw answers, adjustments, step, the client business id) in sessionStorage per user, and `/setup?done=<id>` is the ready screen. Splitting the browser's messages per locale and route (D-069) waits for Step 7: `setup` and `modules` grew the messages chunk from about 24 to 36 KB gzip.
- **Why:** "No cache leak when switching businesses or when two businesses are open in two tabs" (M1 definition of done) with the least machinery; the ready screen survives a reload without state.
- **Rejected:** one shared `QueryClient` with business-keyed queries (an in-flight query can still land in the other business); a business id in a cookie or global store. Details: ARCHITECTURE.md §Data fetching & caching

### D-080 · 2026-09-26 · Settings sections follow permissions and capabilities, and the API enforces both

- **Decision:** Settings of a business has six sections: Business profile (settings.business.view; editing needs .edit), Branches (capability `multi_location` and settings.locations.manage), Team = members + invitations (`has_team` and settings.members.view; changes need .manage), Roles (`has_team` and settings.roles.manage), Customize BizCost (settings.modules.manage) and Language (settings.business.view, read-only without .edit, with a link to the personal account page). A solo business has no team or role screens and a single-location business no branch screens (PRODUCT.md §5). The API refuses the team, role and invitation procedures without `has_team` and the location procedures without `multi_location` (`requireCapability` → CAPABILITY_DISABLED), so capabilities are gates there, not only UI. Customize refuses to turn `has_team` off while other active members or pending invitations exist (`team_in_use`) and `multi_location` off while more than one location exists (`locations_in_use`); Dashboard and Settings cannot be switched off; `vat_registered` is edited in the business profile only. Language is open read-only to members who may see the business (the lead's H1 said .edit): they see which language invitations use.
- **Why:** The lead's H1. A hidden screen whose procedure still answers would leave a team or branches half-working in a business that switched them off.
- **Rejected:** capabilities as UI-only flags (ARCHITECTURE.md said so before Step 6); hiding the Language section from viewers. Details: ARCHITECTURE.md §Permissions, modules & capabilities

### D-081 · 2026-09-26 · The API's own emails: one sender, after the commit

- **Decision:** `packages/api/src/email` (server-only) has an `EmailSender` with `send({ to, subject, html, text })`. The transport comes from the environment: `EMAIL_TRANSPORT=smtp` (nodemailer; locally the stack's Mailpit on `[local_smtp] smtp_port = 54325`) or `RESEND_API_KEY` + `EMAIL_FROM` (the Resend HTTP API, production; not exercised locally). A send is bounded to 10 s in all. Emails are sent only after the transaction that decided them has committed, never inside it. Templates are small bilingual inline-styled HTML + text in TypeScript (`emails` namespace) with the brand of `supabase/templates`; names are escaped and isolated, addresses kept on one line. Bodies, links and tokens are never logged.
- **Why:** The lead's H2. The review showed a send inside the transaction held the instance's only pooled connection and the business's locks: a 2 s provider delay stalled another user's request.
- **Rejected:** react-email or another template framework (heavy for one email); an outbox table and a sender job (no background jobs in M1; ROADMAP.md §Open, with deadline). Details: ARCHITECTURE.md §API & request flow (Step 6), §Auth

### D-082 · 2026-09-26 · Invitations: tokens, limits, preview, and they follow their sender

- **Decision:** A token is 32 random bytes (base64url); only its SHA-256 hex is stored; the link `{APP_URL}/invite/{token}` works 7 days; a resend makes a new token. Limits in the database (`app.invitation_limits`): 20 invitations per business and 40 per inviting user in 24 hours, 3 resends per invitation, and 4 emails per address and business in 24 hours, so cancelling and inviting again does not start over. When the email of a new invitation cannot be sent, the invitation is revoked (it still counts) and the call fails; the web then uses a new id. The public preview returns the business name, the inviter's name, the role template key and, for a custom role, the role name, the masked email, the expiry and, signed in, whether the caller's verified email matches (so the page can say "signed in with another email"); at most 30 previews per invitation an hour, counted without touching the row or writing the audit log. Pending invitations follow their sender: leaving or being removed revokes them; after a role change, role edit or ownership transfer the API revokes those the sender could no longer send; `app.accept_invitation` also requires the sender to still be an active member allowed to invite. Resend and cancel follow "not beyond your own access" like create. The pending list names who sent each invitation. The invitation page is open signed in or not; the way back, the business name and the masked email stay in sessionStorage, and the sign in and sign up pages show them.
- **Why:** The lead's H3, and the review's findings: a removed Admin could come back through an invitation sent to a second address; a Manager could cancel the owner's Admin invitation; one business could send one address 80 emails a day; previews wrote audit rows with the invitee's email from a public GET.
- **Rejected:** `auth.admin.inviteUserByEmail` (D-029); a preview that returns nothing but the business name (the page could not guide a wrong-account visitor); keeping invitations of removed members valid. Details: ARCHITECTURE.md §Auth; DATA_MODEL.md §4 business_invitations

### D-083 · 2026-09-26 · Members keep the email they joined with

- **Decision:** `business_members.email citext NULL` (new migration) holds the account's verified email: set by `app.create_business` and `app.accept_invitation`, filled once for existing account members, and copied again by `me` when the token's email differs (as names are copied, D-065). The members list shows it (`pin_only` staff have none) and "already a member" is checked against it. Account deletion clears it and the display name on every membership of the user, removed ones included (`app.anonymize_my_memberships`, SECURITY DEFINER, only for the caller's own rows and only once the profile is anonymized).
- **Why:** The lead's H4; the review found the email stayed on a deleted user's memberships and went stale after an email change.
- **Rejected:** reading emails from `auth.users` at list time (the API role has no access to schema `auth`, on purpose); a trigger on `auth.users` (writes by the Auth server without a tenant actor). Details: DATA_MODEL.md §4 business_members

### D-084 · 2026-09-26 · Roles: edits stay within the editor's access; M1 shows no sensitive-data switches

- **Decision:** Roles lists the business's roles; the Owner is read-only ("can do everything"). Someone who is not the owner may edit only a role that grants nothing beyond their own access (a Manager allowed to manage roles cannot touch the Admin or Accountant role), and add or remove only keys they hold; the web shows such roles locked with the reason and a lock on each switch the editor cannot change. A key that needs another comes with it (`PERMISSION_NEEDS` in `@bizcost/modules`: `.edit` needs `.view`, `members.manage` needs `members.view`; the API answers VALIDATION, the editor switches both). The editor offers neither the sensitive-data keys (`data.*`) in M1 nor "Manage branches" to a single-location business; a saved role keeps those keys as they are. Editing your own role says so, and taking access away from yourself asks first, naming what you lose. A changed role's summary says what it allows in plain words ("It can see the dashboard, see the team and 3 more things"), not a count. Saving bumps `permissions_version` for the role's members and revokes pending invitations their senders can no longer send.
- **Why:** The lead's H7 with PRODUCT.md §8 and ROADMAP.md (the sensitive-fields section ships with the first module that has sensitive fields; no M1 screen has such fields). The review found a Manager could strip the Admin role, which Admins could not undo, and that "Changed: 8 permissions" read as 8 changes.
- **Rejected:** only checking the changed keys; the sensitive-data group in M1 (it would configure fields no screen has). Details: ARCHITECTURE.md §API & request flow (Step 6); PRODUCT.md §8

### D-085 · 2026-09-26 · Logo uploads go through a registry that Storage checks

- **Decision:** The private bucket `business-files` (migration; 2 MB; PNG, JPEG, WebP; no SVG). `business.logoUploadUrl` (settings.business.edit) first registers the path `{business_id}/logo/{uuidv7}.{ext}` in `app.file_uploads` (at most 10 per business an hour, in the database), then returns Storage's signed upload URL. A trigger on `storage.objects` lets Storage write in this bucket only at an `issued`, unexpired path. `business.setLogo(path)` accepts only an open upload of this business and checks the object (size, stored type, extension, first bytes); the path is then `used`, and a refused, replaced or removed logo's path `discarded` with its object removed. Uploads that expired unused are removed when the business asks for its next upload URL. The profile returns a signed download URL for 10 minutes. The secret-key client stays in `src/admin`, which only the account and business-profile services may import (lint).
- **Why:** The lead's H5. The review showed upload URLs were unlimited and, being valid for 2 hours, could put different bytes back at a removed logo's path, served by an earlier download URL.
- **Rejected:** proxying uploads through the API (the lead chose signed URLs; function body limits); a shorter upload URL lifetime (Storage does not let the caller set it); a background clean-up job (none in M1). Details: ARCHITECTURE.md §Storage; DATA_MODEL.md §4 file_uploads

### D-086 · 2026-09-26 · Customize BizCost reuses the setup rules

- **Decision:** `business.customization` / `business.customize` (settings.modules.manage) switch one module or capability at a time with the same dependency rules as the setup review (`toggleBusinessItem` in `@bizcost/modules`, built on the review's `toggleFromState`): a module turned on brings what it needs, turning off something others need asks first and names them, a capability turned on brings its anchor modules (Team → Employees, VAT → VAT Center…) and turned off takes the modules that require it, and a module that needs VAT while VAT is off is refused. Released and planned sections are shown the same honest way as in the setup (D-075): a planned one is saved and appears when it is ready. Turning off hides and never deletes. The guards of D-080 apply.
- **Why:** The lead's H6: one set of rules for the first setup and every later change.
- **Rejected:** a separate rule set for Customize; deleting data on switch-off. Details: PRODUCT.md §5; ARCHITECTURE.md §Modules

### D-087 · 2026-09-26 · Members: change role, remove, leave, transfer ownership

- **Decision:** The members list shows name, email, role and status. With settings.members.manage: change a role (never to or from Owner, never your own) and remove (status `removed`, never deleted; the owner cannot be removed, only transferred). Anyone but the owner may leave a business (`member.leave`, no permission or capability needed), from the Team list or the settings home. The owner transfers ownership to an active account member after a sign-in or emailed code within 600 s (like account deletion); in one transaction the old owner becomes Admin and the database's owner rule holds. Every change bumps `permissions_version`, so the member's next request has the new access, and a removed member's next request is refused.
- **Why:** The lead's H8. The review found that only people who could manage the team could leave; everyone else had no way out but deleting their account.
- **Rejected:** leaving only through account deletion; removing the owner without a transfer. Details: ARCHITECTURE.md §API & request flow (Step 6)

### D-088 · 2026-09-26 · Shape of the settings pages on the web

- **Decision:** Settings has its own layout under `/b/[businessId]/settings` until the Step 7 shell: from 1024 px a side list of the member's sections beside the section; on phones the list is its own page and each section a page with a way back. A typed link to a section the member may not open says so ("This section isn't open to you") and loads nothing. View-only members see read-only fields with a muted look and one notice. The invite dialog defaults to the Employee role (the least access) and the business's language, keeps LTR emails aligned like the sign-in forms, and brings an error into view on phones. A business the member is no longer in (or that does not exist: the API answers both the same) shows "This business isn't open to you" instead of "Page not found", under one header. Arabic says "إزالة" (remove) for anything hidden or taken away and "حذف" (delete) only for the logo, which is deleted.
- **Why:** The lead's layout brief and the review's UX findings (errors off-screen on phones, two stacked headers, "Page not found" after a removal, "delete" for a branch that is only hidden). Supersedes the "not found" part of D-079.
- **Rejected:** building the Step 7 sidebar now; a modal settings panel. Details: ARCHITECTURE.md §Repo structure

## Web shell (Step 7)

### D-089 · 2026-09-26 · The business shell: sidebar, rail and tab bar, all from `business.context`

- **Decision:** Inside a business (`/b/[businessId]`) the web app has one shell (`src/components/shell`): from 1024 px a 16rem sidebar on the start side with the logo, the business switcher and the sections; from 768 px a 4.5rem icon rail whose names are the links' accessible names and show in a tooltip; below 768 px a tab bar fixed at the bottom, above the home indicator (`viewport-fit=cover` and `env(safe-area-inset-bottom)`), with tabs at least 44 px tall and the active tab tinted like the mockups. The top bar keeps the language and the account menu at the top end on every size, and the switcher on phones and tablets. The sections come only from `business.context.modules` (released, enabled, permitted): the shell names no module. A manifest nav entry now has a `group`: `main` entries come in manifest order, `system` ones (Settings) after them (the sidebar's foot, the last tab), so later modules land before Settings. The tab bar has 5 places, puts the rest under "More", and shows "+" only when a module has "+" actions; an icon name the web does not know gets a neutral icon. `ModuleGate` gives a module's page its "turned off" or "not open to you" state from the same data (the Dashboard uses it for `dashboard.home.view`). Every settings page keeps Settings marked; the settings' own section list moves to 1280 px, beside the sidebar. Pages outside a business (home, account, Smart Setup) keep the plain top bar (route group `(main)`), and "This business isn't open to you" shows under it. `business-frame.tsx` is the business layout's only client entry, so the bundler does not copy the top bar into a second chunk.
- **Why:** The lead's I1/I2 and PRODUCT.md §5 ("nav, tabs and '+' show only released and enabled modules"), with no shell change when a module ships: nav.test.ts releases Orders in a test registry and it appears, before Settings, only for members with its permission. The account and language at the top end is the one place that phones can also have.
- **Rejected:** the account menu in the sidebar's foot (it would move between sizes); a hard-coded nav list; icons loaded by name at run time (lucide's dynamic import map adds a chunk per icon); a collapsible desktop sidebar (not needed with two sections). Details: ARCHITECTURE.md §Repo structure, §Modules

### D-090 · 2026-09-26 · The Dashboard's getting-started checklist

Refined by D-093: the invite and branch steps are left out for members who joined after the milestone, the checklist comes from the server with the page, and the hidden card is remembered in a cookie.

- **Decision:** `dashboard.checklist` (business procedure, module `dashboard`, `dashboard.home.view`) returns the steps the member can act on, in order, each `done` or not from the business's data (one statement). **profile** (settings.business.edit) is done when the business has its name in Arabic (`legal_name_ar`, or the name itself written in Arabic letters) and a logo; it lists what is `missing`. **trn** (settings.business.edit, only with `vat_registered`) is done when the TRN is saved. **invite** (settings.members.manage, only with `has_team`) is done when another member is active or an invitation is waiting and not expired. **location** (settings.locations.manage, only with `multi_location`) is done when a second branch exists. The rules are pure (`checklistItemIds`, `checklistItem` in `@bizcost/modules`), so the page also sizes its placeholder with them. A member who can act on no step gets none (and the database is not read): their Dashboard is the welcome and "About your business". The page shows "n of m done" with a progress bar, and each open step opens its settings section. When every step is done, a compact "You're all set" takes the list's place; it can be hidden (remembered in localStorage per user and business), and a step that opens again always shows. Every successful mutation in the business, except the account's own, refetches the checklist, so it is current when the Dashboard opens again.
- **Why:** PRODUCT.md §10 and the lead's I3. An invitation waiting counts, so an owner who has just invited is not asked again; an expired one does not, so the step comes back.
- **Rejected:** removing a step once done (the progress would count backwards); cards for sections that are not released (§9); the checklist inside `business.context` (only the Dashboard needs it). Details: PRODUCT.md §10

### D-091 · 2026-09-26 · Page states and loading

Refined by D-093 and D-094: the Dashboard waits on nothing below its layout, the business's loading state is the shell with its real controls, and "Page not found" inside a business stays in the shell.

- **Decision:** One `StatePanel` (an icon, a title that is the page's h1, a line of help, a way on) for "not open to you" (403: a settings section without its permission or capability, the Dashboard without `dashboard.home.view`), "turned off" (a released module the business switched off, with a link to Customize BizCost for members who may manage it), "This business isn't open to you", "Page not found" and unexpected errors; business pages have their own error boundary inside the shell. Loading uses `loading.tsx`: `b/loading.tsx` is the shell's frame with placeholders at the real sizes (a server component, so it adds no code); the Dashboard's and the settings' loading files render the page's own first state (what the shell already knows, placeholders for what loads), so nothing moves when the page arrives. The route group `(home)` gives the Dashboard its own layout and loading file.
- **Why:** The lead's I4: one look for every state, a way on from each, and no layout shift.
- **Rejected:** "Page not found" for a section the member may not open; spinners.

### D-092 · 2026-09-26 · The browser gets only its page's language and messages

Refined by D-096: messages in another language than the page's are never added (the page is rendered again), and a lint rule keeps the bundled translations out of browser code.

- **Decision:** Supersedes the bundling part of D-069 and D-079. The browser code holds no translations: the server passes them as props, in the page's language only. The root layout gives `common` and `errors`; the layout or page of a route adds what it needs with `<Messages specs>` (`src/lib/i18n/route-messages.ts`): sign-in pages `auth`; the account `account` and `auth`; Smart Setup `setup` and `modules`; the Dashboard `dashboard` and the setup's statements; Settings `settings`, and each section what it uses (e.g. Customize `setup` and `modules`). A spec is a whole namespace or parts of it (`pickMessages`); `emails` never reaches the browser. The page's i18next instance (`createI18nInstance`) takes more messages as the user moves on (`addMessages`, merged, idempotent); a loading file's messages come from its layout. A language switch saves the cookie (and the profile) and renders again on the server (`router.refresh()`): the new language arrives with its messages and a new instance, with no page reload, and "Language changed." is said in the new language after the switch. In `@bizcost/i18n` only `resources.ts` imports the JSON files, and browser code never imports it (`namespaces.ts`, `instance.ts`, `terminology.ts` are free of them); `terminologyKey` and app-core's `formMessage` take a `has(key)` function. A test reads each route's code for the keys it translates and checks the route's messages hold them; e2e checks that the page's code holds no translation and its HTML only its language. Measured (gzip first-load JS of the route's HTML, production build): /login 349.8 → 325.0 KB; /b/[id] 375.8 → 355.9 KB with the new shell and checklist; /b/[id]/settings 393.5 → 369.6 KB; /b/[id]/settings/business 487.0 → 462.9 KB (after the review fixes, D-093–D-096; the same in Arabic). The HTML grows by the page's messages (gzip: about 4 KB on /login, 5–6 KB on the Dashboard, 8–10 KB on a settings page); JS and HTML together, each page is 8–21 KB lighter.
- **Why:** D-069 deferred it to Step 7: every page loaded a 27 KB gzip chunk with both languages of every namespace, the API's emails included.
- **Rejected:** a JS chunk per language and namespace loaded with `import()` (a loading step, and keys flashing during a language switch); a locale segment in URLs (D-067). Details: ARCHITECTURE.md §i18n & RTL

### D-093 · 2026-09-26 · The Dashboard arrives complete; milestones a member joined after are not theirs

- **Decision:** Refines D-090 and D-091 after the Step 7 review. The Dashboard's `(home)/layout.tsx` loads `dashboard.checklist` through the server caller, in parallel with the business (the API answers FORBIDDEN, read as none, to members without `dashboard.home.view`, and no steps, without a database read, to members who can act on none), and reads whether "You're all set" was hidden; both reach the page through a client context, and the checklist seeds the business's query cache as `initialData`. The page itself waits on nothing, so a full load shows the Dashboard in its final form, never a checklist placeholder that is then replaced. "You're all set" hidden is remembered in the cookie `bz_all_set_hidden` (path `/b/<id>`, the ids of the users who hid it, one year), which the server reads: a member who hid it never sees it appear and go. The first team member and the second branch are milestones: a member who joined after one was reached never gets that step (`teamBeforeMember`: someone was a member before them; `secondBranchBeforeMember`: the second live branch is older than their membership). So only the business's first member gets "Invite your first team member", and a manager who joined a business with two branches has no branch step. When the milestone is lost again (the others left, a branch was removed), the step is open and shows to everyone who can act on it. The profile and TRN steps keep D-090's rule (the data has no time they were done). A member whose role lacks the Dashboard lands on their first section (the layout redirects; the page does the same if access changes while it is open). Done steps are quiet (a light disc; the "Done" tag is text on the card at 5:1), so the open ones keep the eye.
- **Why:** The lead's I3 ("only when … no other active member", "only when … exactly one location") and the review: every full load showed the checklist placeholder and then moved the page, owners who hid the card saw it flash, an admin saw "Invite your first team member ✓" and a manager "You're all set" on their first visit, and a role without the Dashboard always landed on a 403.
- **Rejected:** removing done steps for everyone (the owner's "n of m" would count backwards); `localStorage` for the hidden card (the server cannot read it); loading the checklist in the page (its placeholder showed for about 300 ms while React revealed the page). Details: PRODUCT.md §10; ARCHITECTURE.md §Data fetching & caching

### D-094 · 2026-09-26 · Shell accessibility: focus, bars, motion, titles and states

- **Decision:** After a navigation inside the business shell, or into a business from elsewhere (another business, the account), the focus goes to the page area (`main#main`) when the link that was used is gone; a focus that is still somewhere (the sidebar link) and the first page of a visit (hydration) are left alone. "Hide" on "You're all set" moves the focus to "About your business". `html` has `scroll-padding` for the sticky top bar and, below 768 px, the tab bar, so a focused control never ends up under them (WCAG 2.4.11, also at 200 % zoom). `prefers-reduced-motion` stops animations, transitions and smooth scrolling. The rail's tooltip is a visual label only (hidden from screen readers and not the link's description; always a controlled tooltip). The sidebar and the settings side list are plain containers around their `nav` landmarks. Menu items are 44 px tall on touch screens. The account button's name includes the visible name. A state that replaces a page (403, turned off, not found, errors) also sets the document's title, so the route announcer reads the state. "Page not found" inside a business stays in the shell (`[businessId]/not-found.tsx`, with catch-all routes for other addresses and for Settings); the app's own "Page not found" uses the plain top bar for signed-in users. The way on from a state goes to another section of the nav ("Go to <section>", derived from `business.context`, never a named module). While a business loads, `b/loading.tsx` shows the shell with its real logo, switcher (the business being opened), language and account, inert, with as many section placeholders as the released modules have (`navSlots`). Inside a business the account menu has no "Home" item, so an Arabic page has one «الرئيسية» (the Dashboard).
- **Why:** The Step 7 accessibility review (focus lost to `<body>` after navigations and after "Hide", focus hidden under the bars, tooltips read twice, an unnamed complementary region, 35–40 px menu items, a 403 announced with its section's title, two links named «الرئيسية» going to different places) and the visual review (a switch or a mistyped address dropped the whole shell).
- **Rejected:** focusing the page's h1 (pages differ; the page area is always there); a focusable loading shell (its focus is lost when the business arrives); removing `b/loading.tsx` to keep the old shell during a switch (the lead's I4 asks for the business's loading state).

### D-095 · 2026-09-26 · Names in either script, fonts, and page breakpoints beside the sidebar

- **Decision:** A business or person name is never cut where it is the page's subject: the Dashboard's heading wraps (`text-balance`), the switcher's menu (20rem) shows up to two lines per name, and the truncated trigger has the full name as its tooltip and accessible name. A name keeps its own direction but sits at the page's start (`PersonName`: a block at the page's start, the name inside with `dir="auto"`), so "Moon Café" in the Arabic UI lines up with its role. The two real faces come before any metric fallback on every page: the root layout sets the Latin variable to IBM Plex Sans alone (next/font's Arial fallback would otherwise draw Arabic names on English pages), and the Arabic face's fallback follows both. Layouts in the page area switch at 1280 px (`xl`) rather than 1024 px, since the sidebar takes 16rem from 1024 px: the Dashboard puts the checklist beside "About your business" from 1280 px, and the settings lists use three columns from 1280 px. The settings side list no longer repeats the business's name and is marked like the sidebar. The Arabic name's hint describes the legal name (as on the trade licence) instead of promising where BizCost shows it; whether the shell should show the Arabic name and the logo is for the owner (ROADMAP.md §Open).
- **Why:** The Step 7 visual review: the business name could not be read in full on a phone or at 1024 px, Arabic names on English pages were drawn in Arial, English names in the Arabic UI sat on the wrong edge, and 1024–1279 px was cramped.
- **Rejected:** `adjustFontFallback: false` (Turbopack still puts the fallback in the variable); container queries (the sidebar's width follows the viewport, so `xl` says the same more simply).

### D-096 · 2026-09-26 · Messages from before a language change, and the guard on bundled translations

- **Decision:** Refines D-092. `AddMessages` never adds messages in another language than the page's: such messages come from a layout the router kept from before a language change (a link prefetched in the old language). It then shows nothing and renders the page again on the server (`router.refresh()`), so untranslated keys never show. ESLint forbids `resources`, `hasMessage`, `pickMessages` and `createI18n` from `@bizcost/i18n` in `apps/web` except in `lib/i18n/server.ts`, `lib/i18n/messages.tsx`, `app/layout.tsx` and tests, so browser code cannot bundle every translation again. The route-messages guard checks the home page against the root messages only.
- **Why:** Found while verifying Step 7: after a language change on another device (the profile then differs from the cookie), opening the Dashboard from Settings showed "checklist.title"; and the review noted that only `/login` was guarded against bundled translations.
- **Rejected:** sending both languages' messages (D-092); a post-build scan of every chunk (the lint rule stops the import before the build).
