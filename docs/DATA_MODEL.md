...
- Files: private Storage bucket `business-files` (migration `settings_security`: private, 2 MB, PNG/JPEG/WebP), object path `{business_id}/{entity}/{uuidv7}.{ext}`; access rules in ARCHITECTURE.md §Storage. M1 use: business logo (`businesses.logo_path`), uploaded through the registry `file_uploads` (§4). Generic attachments table is PLANNED (§6).
...
- At most 10 per business in an hour (trigger `app.file_upload_limits()`, advisory lock per business, `BZ429`).
